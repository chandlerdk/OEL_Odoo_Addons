...
class EBizUploadInvoices(models.Model):
    ...
    logs_line = fields.One2many('ebiz.log.invoice', 'sync_log_id', copy=True,  domain=lambda self: self._get_logs_domain())
    ...
    def js_flush_customer(self, ecom_side=None):
        rec = self.env['ebiz.upload.invoice'].search([])
        # if rec:
    ...
    def create_default_records(self):
        ...
        self.env["ebiz.list.invoice"].search([]).unlink()
        list_invoice = self.env['ebiz.list.invoice']
        ...
        if self.ebiz_profile_id == '0':
            ...
            list_of_invoices = account_obj.search([("move_type", "=", "out_invoice"), ('partner_id.ebiz_profile_id', '=', int(self.ebiz_profile_id))])
        list_of_upload_invoice = list_invoice.search([])
        if (len(list_of_invoices)) != (len(list_of_upload_invoice)):
            list_of_dict = []
            for invoice in list_of_invoices:
                is_order = list_invoice.search([('invoice', '=', invoice.id)])
                if not is_order:
    ...
    def read(self, fields, load='_classic_read'):
        ...
        resp = super(EBizUploadInvoices, self).read(fields, load)
        return resp
    ...
    def upload_invoice(self, *args, **kwargs):
        try:
            ...
            filter_record = self.env['ebiz.list.invoice'].browse(res_ids).exists()
            if not filter_record:
                ...
                return record.invoice.sync_multi_customers_from_upload_invoices(list_ids)
    ...
    def export_orders(self, *args, **kwargs):
        ...
        return {
            ...
            'url': 'web/content/?model=bill.excel&field=excel_file&download=true&id=%s&filename=Invoices.xls' % (
                export_id.id),
            'target': 'new', }
    ...
    def export_logs(self, *args, **kwargs):
        ...
        for record in kwargs['values']:
            res_ids.append(record['id'])
        filter_record = self.env['ebiz.log.invoice'].browse(res_ids).exists()
        if not filter_record:
            raise UserError('Please select a record first!')
        ...
        worksheet, workbook, header_style, text_center = self.env['ebizcharge.instance.config'].export_generic_method(sheet_name='Invoice Logs',
                                                                                           columns=column_names)
        i = 4
        for record in filter_record:
            worksheet[0].write(i, 1, record.invoice.name or '', text_center)
        ...
        workbook.save(fp)
        export_id = self.env['bill.excel'].create(
            {'excel_file': base64.encodestring(fp.getvalue()), 'file_name': 'Invoice Logs.xls'})

        return {
            'type': 'ir.actions.act_url',
            'url': 'web/content/?model=bill.excel&field=excel_file&download=true&id=%s&filename=Invoice Logs.xls' % (
                export_id.id),
            'target': 'new', }
