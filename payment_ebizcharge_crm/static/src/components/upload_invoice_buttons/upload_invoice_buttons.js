...
class UploadInvoiceButtons extends ListRenderer {
    // setup method
    setup() {
        super.setup();
        this.selection = [];
   ...
   toggleRecordSelection(record) {
    ...
    async UploadInvoiceUpload (ev) {
            ...
            const params = Object.fromEntries(
             ...
             const action = await this.rpc("/web/dataset/call_kw/ebiz.upload.invoice/upload_invoice", {
                model: 'ebiz.upload.invoice',
    ...
    async UploadInvoiceExportList (ev) {
            const res_ids = this.selection.map((record) => record.data);
            const hash = window.location.hash.substring(1);
            // Parse the hash into an object
            const params = Object.fromEntries(
             ...
             const action = await this.rpc("/web/dataset/call_kw/ebiz.upload.invoice/export_orders", {
                model: 'ebiz.upload.invoice',
    ...
    async UploadInvoiceClearLogs (ev) {
            const res_ids = this.selection.map((record) => record.data);
            const hash = window.location.hash.substring(1);
            // Parse the hash into an object
            const params = Object.fromEntries(
             ...
             const action = await this.rpc("/web/dataset/call_kw/ebiz.upload.invoice/clear_logs", {
                model: 'ebiz.upload.invoice',
                method: 'clear_logs',
                args : [[parseInt(params['id'])]],
                kwargs: {"values":res_ids}
    ...
    async UploadInvoiceExportLogs (ev) {
            const res_ids = this.selection.map((record) => record.data);
            const hash = window.location.hash.substring(1);
            // Parse the hash into an object
            const params = Object.fromEntries(
             ...
             const action = await this.rpc("/web/dataset/call_kw/ebiz.upload.invoice/export_logs", {
                model: 'ebiz.upload.invoice',
                method: 'export_logs',
                args : [[parseInt(params['id'])]],
                kwargs: {"values":res_ids}
...
UploadInvoiceButtons.template = "payment_ebizcharge_crm.UploadInvoiceButtons";
export class UploadInvoiceButtonsX2ManyField extends X2ManyField {
    setup() {
        super.setup();
    }
}

UploadInvoiceButtonsX2ManyField.components = { ...X2ManyField.components, ListRenderer: UploadInvoiceButtons };

export const uploadInvoiceButtonsX2ManyField = {
    ...x2ManyField,
    component: UploadInvoiceButtonsX2ManyField,
    additionalClasses: [...x2ManyField.additionalClasses || [], "o_field_one2many"],
};

registry.category("fields").add("upload_invoice_buttons", uploadInvoiceButtonsX2ManyField);
