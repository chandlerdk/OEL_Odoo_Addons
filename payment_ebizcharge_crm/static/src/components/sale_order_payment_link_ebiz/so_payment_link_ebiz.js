...
class SalePaymentLinkEBiz extends ListRenderer {
    // setup method
    setup() {
        super.setup();
        this.selection = [];
    ...
    async showSoExport (ev) {
            ...
            const params = Object.fromEntries(
             ...
             const action = await this.rpc("/web/dataset/call_kw/sale.order.payment.link.bulk/export_sale_order", {
                model: 'sale.order.payment.link.bulk',
                method: 'export_sale_order',
    ...
    async removeSoPaylink (ev) {
            ...
            const params = Object.fromEntries(
             ...
             const action = await this.rpc("/web/dataset/call_kw/sale.order.payment.link.bulk/action_remove_payment_link", {
                model: 'sale.order.payment.link.bulk',
                method: 'action_remove_payment_link',
                args : [[parseInt(params['id'])]],
                kwargs: {"values":res_ids}
            })
            await this.env.services.action.doAction(action, {
    ...
    async generateSoPaylink (ev) {
            ...
            const params = Object.fromEntries(
             ...
             const action = await this.rpc("/web/dataset/call_kw/sale.order.payment.link.bulk/action_generate_payment_link", {
                model: 'sale.order.payment.link.bulk',
...
SalePaymentLinkEBiz.template = "payment_ebizcharge_crm.SalePaymentLinkEBiz";

export class SalePaymentLinkX2ManyField extends X2ManyField {
setup() {
        super.setup();
    }
}

SalePaymentLinkX2ManyField.components = {...X2ManyField.components, ListRenderer: SalePaymentLinkEBiz};

export const salePaymentLinkX2ManyField = {
    ...x2ManyField,
    component: SalePaymentLinkX2ManyField,
    additionalClasses: [...x2ManyField.additionalClasses || [], "o_field_one2many"],
};

registry.category("fields").add("so_payment_link_ebiz", salePaymentLinkX2ManyField);
